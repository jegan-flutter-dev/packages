...
class ImagePickerMetaDataUtilTests: XCTestCase {
  ...
  func testConvertImageToData_Basic() throws {
    let imageJPG = try XCTUnwrap(UIImage(data: ImagePickerTestImages.jpgTestData))

    // ✅ Case 1: JPEG with quality (existing)
    let data05 = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .jpeg, quality: 0.5)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(data05)), .jpeg)

    // ✅ Case 2: JPEG boundary quality (forces extra lines)
    let data00 = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .jpeg, quality: 0.0)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(data00)), .jpeg)

    // ✅ Case 3: PNG branch (existing)
    let dataNil = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .png, quality: nil)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(dataNil)), .png)
  }

  func testConvertImageToData_Advanced() throws {
    let imageJPG = try XCTUnwrap(UIImage(data: ImagePickerTestImages.jpgTestData))

    // ✅ Case 4: PNG with explicit quality (hits warning/ignored-quality lines)
    let data07 = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .png, quality: 0.7)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(data07)), .png)

    // ✅ Case 5: GIF branch (VERY IMPORTANT for uncovered lines)
    let dataGIF = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .gif, quality: 0.6)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(dataGIF)), .jpeg)

    // ✅ Case 6: Default fallback (.other)
    let dataOther = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .other, quality: nil)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(dataOther)), .jpeg)

    // ✅ Case 7: Another fallback with quality (forces deeper default coverage)
    let data08 = ImagePickerMetaDataUtil.convertImage(imageJPG, using: .other, quality: 0.8)
    XCTAssertEqual(
      try ImagePickerMetaDataUtil.getImageMIMEType(from: XCTUnwrap(data08)), .jpeg)
  }
  ...
  func testImageWithMetadata_InvalidDataReturnsNil() throws {
    ...
    // Case 3: Valid image WITHOUT metadata (success path)
    let image = try XCTUnwrap(UIImage(systemName: "circle"))
    let validData = try XCTUnwrap(image.jpegData(compressionQuality: 1.0))
    let result4 = ImagePickerMetaDataUtil.image(from: validData, with: [:])
    XCTAssertNotNil(result4)
  ...
  func testImageWithMetadata_CorruptedHeader() throws {
    ...
    // Case 3: Valid image with metadata (covers metadata handling branch)
    let image = try XCTUnwrap(UIImage(systemName: "circle"))
    let validData = try XCTUnwrap(image.jpegData(compressionQuality: 1.0))
    let metadata: [String: Any] = [
      kCGImagePropertyOrientation as String: 1
