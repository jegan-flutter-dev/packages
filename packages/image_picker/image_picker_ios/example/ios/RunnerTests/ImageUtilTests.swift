...
class ImageUtilTests: XCTestCase {
  ...
  private func colorsAreEqual(_ colorString1: String?, _ colorString2: String) -> Bool {
    guard let colorString1 else { return false }
    let components1 = colorString1.split(separator: " ").compactMap { Double($0) }
    let components2 = colorString2.split(separator: " ").compactMap { Double($0) }

    guard components1.count == 4 && components2.count == 4 else { return false }

    for index in 0..<4 where abs(components1[index] - components2[index]) > 0.01 {
      return false
    }
    return true
  }
  ...
  private func colorStringAtPixel(
    _ image: UIImage,
    xCoordinate: Int,
    yCoordinate: Int
  ) -> String? {
    ...
    context.draw(
      cgImage,
      in: CGRect(
        x: CGFloat(-xCoordinate), y: CGFloat(-yCoordinate), width: CGFloat(cgImage.width),
  ...
  func testScaledImage_ShouldBeScaled() {
    ...
    let color = colorStringAtPixel(scaledImage, xCoordinate: 0, yCoordinate: 0)
    XCTAssertTrue(
