...
class ImagePickerPluginTests: XCTestCase {
  ...
  func testPickVideo_SetsCorrectDuration() {
    ...
    plugin.pickVideo(source: SourceSpecification(type: .camera, camera: .rear), maxDurationSeconds: 10) { _ in }
  ...
  func testPickMedia_SetsCorrectContext() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider())
    let options = MediaSelectionOptions(maxSize: MaxSize(width: 10, height: 20), imageQuality: 70, requestFullMetadata: false, allowMultiple: true, limit: 3)
  ...
  func testPickMedia_Single_SetsCorrectContext() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider())
    let options = MediaSelectionOptions(maxSize: MaxSize(width: nil, height: nil), imageQuality: nil, requestFullMetadata: true, allowMultiple: false, limit: nil)
    plugin.pickMedia(mediaSelectionOptions: options) { _ in }
    XCTAssertNotNil(plugin.callContext)
  ...
  func testPickMedia_Multiple_WithLimit_SetsCorrectContext() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider())
    let options = MediaSelectionOptions(maxSize: MaxSize(width: nil, height: nil), imageQuality: nil, requestFullMetadata: false, allowMultiple: true, limit: 10)
    plugin.pickMedia(mediaSelectionOptions: options) { _ in }
    XCTAssertNotNil(plugin.callContext)
  ...
  func testGetDesiredImageQuality() {
    ...
    let testCases: [(input: Double?, expected: Double)] = [
      (nil, 1.0), (100.0, 1.0), (50.0, 0.5), (0.0, 0.0), (-1.0, 1.0), (101.0, 1.0), (-100.0, 1.0), (1000.0, 1.0),
    ]
    for testCase in testCases {
      XCTAssertEqual(plugin.getDesiredImageQuality(testCase.input), testCase.expected, "Failed for input: \(String(describing: testCase.input))")
    }
  }
  ...
  func testCheckPhotoAuthorization_HandlesAllStatuses() {
    ...
    plugin.checkPhotoAuthorization(with: UIViewController())
    if #available(iOS 14, *) { mockHandler.photoLibraryAuthorizationStatusResult = .limited }
    plugin.checkPhotoAuthorization(with: UIViewController())
    waitForExpectations(timeout: 1)
  }
  ...
  func testLaunchUIImagePicker_WithImages_SetsConfiguration() {
    ...
    plugin.setImagePickerControllerOverrides([mockPicker])
    plugin.launchUIImagePicker(with: SourceSpecification(type: .gallery, camera: .rear), context: context)
    if #available(iOS 14.0, *) { XCTAssertTrue(mockPicker.mediaTypes.contains(UTType.image.identifier)) }
  }
  ...
  func testPickImage_FromCamera_SetsCorrectContext() {
    ...
    let plugin = ImagePickerPlugin(viewProvider: viewProvider, deviceCapabilityHandler: mockHandler)
    plugin.setImagePickerControllerOverrides([MockUIImagePickerController()])
    plugin.pickImage(source: SourceSpecification(type: .camera, camera: .rear), maxSize: MaxSize(width: 100, height: 100), imageQuality: 80, requestFullMetadata: false) { _ in }
    XCTAssertNotNil(plugin.callContext)
    XCTAssertEqual(plugin.callContext?.maxSize?.width, 100)
  ...
  func testPickImage_FromCamera_WhenDenied_ReturnsError() {
    ...
    plugin.pickImage(source: SourceSpecification(type: .camera, camera: .rear), maxSize: MaxSize(width: nil, height: nil), imageQuality: nil, requestFullMetadata: false) { result in
      if case .failure(let error as PigeonError) = result {
  ...
  func testPickVideo_FromGallery_SetsCorrectContext() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider(viewController: UIViewController()))
    plugin.pickVideo(source: SourceSpecification(type: .gallery, camera: .rear), maxDurationSeconds: 20) { _ in }
    XCTAssertNotNil(plugin.callContext)
  ...
  func testScaledImage_WithMaxWidth_ScalesCorrectly() {
    let image = UIImage(data: ImagePickerTestImages.jpgTestData)!
    let scaled = ImagePickerImageUtil.scaledImage(image, maxWidth: 5, maxHeight: nil, isMetadataAvailable: false)
    XCTAssertEqual(scaled.size.width, 5)
    XCTAssertEqual(scaled.size.height, 3)
  }
  func testScaledImage_WithMaxHeight_ScalesCorrectly() {
    let image = UIImage(data: ImagePickerTestImages.jpgTestData)!
    let scaled = ImagePickerImageUtil.scaledImage(image, maxWidth: nil, maxHeight: 4, isMetadataAvailable: false)
    XCTAssertEqual(scaled.size.height, 4)
    XCTAssertEqual(scaled.size.width, 7)
  }
  func testScaledImage_WithBoth_ScalesToFit() {
    let image = UIImage(data: ImagePickerTestImages.jpgTestData)!
    let scaled = ImagePickerImageUtil.scaledImage(image, maxWidth: 6, maxHeight: 6, isMetadataAvailable: false)
  ...
  func testPickMedia_FromGallery_SetsCorrectContext() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider(viewController: UIViewController()))
    let options = MediaSelectionOptions(maxSize: MaxSize(width: 50, height: 50), imageQuality: 90, requestFullMetadata: true, allowMultiple: false, limit: nil)
    plugin.pickMedia(mediaSelectionOptions: options) { _ in }
    XCTAssertNotNil(plugin.callContext)
  ...
  func testErrorNoPhotoAccess_Denied_Code() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider())
    let expectation = self.expectation(description: "Denied error")
    plugin.callContext = ImagePickerMethodCallContext { _, error in
      XCTAssertEqual((error as? PigeonError)?.code, "photo_access_denied")
  ...
  func testErrorNoPhotoAccess_Restricted_Code() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider())
    let expectation = self.expectation(description: "Restricted error")
    plugin.callContext = ImagePickerMethodCallContext { _, error in
      XCTAssertEqual((error as? PigeonError)?.code, "photo_access_restricted")
  ...
  func testPickImageMessageHandler_Success() {
    ...
    ImagePickerApiSetup.setUp(binaryMessenger: messenger, api: plugin)
    let handler = messenger.handlers["dev.flutter.pigeon.image_picker_ios.ImagePickerApi.pickImage"]
    let args: [Any?] = [SourceSpecification(type: .gallery, camera: .rear), MaxSize(width: 100, height: 100), Int64(80), true]
  ...
  func testPickMultiImageMessageHandler_Success() {
    ...
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider(viewController: UIViewController()))
    ImagePickerApiSetup.setUp(binaryMessenger: messenger, api: plugin)
    let handler = messenger.handlers["dev.flutter.pigeon.image_picker_ios.ImagePickerApi.pickMultiImage"]
  ...
  func testPickVideoMessageHandler_Success() {
    ...
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider(viewController: UIViewController()))
    ImagePickerApiSetup.setUp(binaryMessenger: messenger, api: plugin)
    let handler = messenger.handlers["dev.flutter.pigeon.image_picker_ios.ImagePickerApi.pickVideo"]
  ...
  func testPickMultiVideoMessageHandler_Success() {
    ...
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider(viewController: UIViewController()))
    ImagePickerApiSetup.setUp(binaryMessenger: messenger, api: plugin)
    let handler = messenger.handlers["dev.flutter.pigeon.image_picker_ios.ImagePickerApi.pickMultiVideo"]
  ...
  func testPickMediaMessageHandler_Success() {
    ...
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider(viewController: UIViewController()))
    ImagePickerApiSetup.setUp(binaryMessenger: messenger, api: plugin)
    let handler = messenger.handlers["dev.flutter.pigeon.image_picker_ios.ImagePickerApi.pickMedia"]
    ...
    let message = MessagesPigeonCodec.shared.encode([options])
    let expectation = self.expectation(description: "Reply called")
    handler!(message) { reply in
      let decoded = MessagesPigeonCodec.shared.decode(reply!) as! [Any?]
  ...
  func testMessageHandler_Failure_GenericError() {
    ...
    ImagePickerApiSetup.setUp(binaryMessenger: messenger, api: plugin)
    let handler = messenger.handlers["dev.flutter.pigeon.image_picker_ios.ImagePickerApi.pickImage"]
    let message = MessagesPigeonCodec.shared.encode([SourceSpecification(type: .gallery, camera: .rear), MaxSize(), nil, false])
    let expectation = self.expectation(description: "Reply called generic error")
    handler!(message) { reply in
      let decoded = MessagesPigeonCodec.shared.decode(reply!) as! [Any?]
  ...
  func testDeviceCapabilityHandler_RequestAccess() {
    ...
    let expCamera = expectation(description: "Camera access")
    let expPhoto = expectation(description: "Photo access")
    handler.requestCameraAccess { granted in XCTAssertTrue(granted); expCamera.fulfill() }
    handler.requestPhotoLibraryAuthorization { status in XCTAssertEqual(status, .authorized); expPhoto.fulfill() }
  ...
  func testSendCallResult_WithEmptyPaths_ReturnsEmptyArray() {
    ...
    plugin.callContext = ImagePickerMethodCallContext { paths, error in XCTAssertEqual(paths?.count ?? 0, 0); expectation.fulfill() }
    plugin.sendCallResult(pathList: [])
    waitForExpectations(timeout: 1)
  }
  func testCameraDevice_DefaultFallback() {
    let plugin = ImagePickerPlugin(viewProvider: StubViewProvider())
    XCTAssertEqual(plugin.cameraDevice(for: SourceSpecification(type: .gallery, camera: .rear)), .rear)
  }
  ...
  func testShowCamera_WhenDeviceNotAvailable_ReturnsNil() {
    ...
    plugin.callContext = ImagePickerMethodCallContext { paths, _ in XCTAssertNil(paths); expectation.fulfill() }
    plugin.showCamera(.rear, with: UIImagePickerController())
    waitForExpectations(timeout: 1)
  }
  ...
  func testCheckCameraAuthorization_UnknownDefault() {
    ...
    plugin.callContext = ImagePickerMethodCallContext { _, _ in expectation.fulfill() }
    plugin.checkCameraAuthorization(with: UIImagePickerController(), camera: .rear)
    waitForExpectations(timeout: 1)
  }
  ...
  class MockUIImagePickerController: UIImagePickerController {
    ...
    override var isBeingPresented: Bool { return mockIsBeingPresented }
    private var _sourceType: UIImagePickerController.SourceType = .photoLibrary
    override var sourceType: UIImagePickerController.SourceType { get { return _sourceType } set { _sourceType = newValue } }
    override var cameraDevice: UIImagePickerController.CameraDevice { get { return .rear } set {} }
  }
  class StubViewProvider: ViewProvider {
    private var viewProviderViewController: UIViewController?
    init(viewController: UIViewController? = nil) { self.viewProviderViewController = viewController }
    var viewController: UIViewController? { return viewProviderViewController }
  }
  class MockDeviceCapabilityHandler: DeviceCapabilityHandler {
    ...
    func isSourceTypeAvailable(_ sourceType: UIImagePickerController.SourceType) -> Bool { isSourceTypeAvailableCalled = true; return isSourceTypeAvailableResult }
    func isCameraDeviceAvailable(_ cameraDevice: UIImagePickerController.CameraDevice) -> Bool { return isCameraDeviceAvailableResult }
    func cameraAuthorizationStatus() -> AVAuthorizationStatus { return cameraAuthorizationStatusResult }
    func requestCameraAccess(completionHandler: @escaping (Bool) -> Void) { requestCameraAccessCalled = true; completionHandler(requestCameraAccessResult) }
    func photoLibraryAuthorizationStatus() -> PHAuthorizationStatus { photoLibraryAuthorizationStatusCalled = true; return photoLibraryAuthorizationStatusResult }
    func requestPhotoLibraryAuthorization(_ handler: @escaping (PHAuthorizationStatus) -> Void) { requestPhotoLibraryAuthorizationCalled = true; handler(requestPhotoLibraryAuthorizationStatusResult) }
  }
  ...
  class TestBinaryMessenger: NSObject, FlutterBinaryMessenger, @unchecked Sendable {
    ...
    func setMessageHandlerOnChannel(_ channel: String, binaryMessageHandler handler: FlutterBinaryMessageHandler? = nil) -> FlutterBinaryMessengerConnection {
      if let handler = handler { handlers[channel] = handler } else { handlers.removeValue(forKey: channel) }
      return 0
    }
