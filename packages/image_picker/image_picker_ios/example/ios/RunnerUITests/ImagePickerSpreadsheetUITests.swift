...
class ImagePickerSpreadsheetUITests: XCTestCase {
  ...
  func testMultiSelectFunctionality() {
    ...
    let doneButton =
      app.buttons["Add"].firstMatch.exists ? app.buttons["Add"] : app.buttons["Done"]
    ...
    XCTAssertTrue(
      ...
      "No image was picked in multi-select mode")
  }
  ...
  func testStandardVideoSelection() {
    ...
    if videoItem.exists {
      videoItem.tap()
      let pickedPredicate = NSPredicate(format: "label CONTAINS 'Picked'")
      XCTAssertTrue(
        app.staticTexts.matching(pickedPredicate).firstMatch.waitForExistence(
          timeout: elementWaitingTime))
    }
  }
  ...
  func testMixedMediaSelection() {
    ...
    let pickedImage = app.images["image_picker_example_picked_image"].firstMatch
    XCTAssertTrue(pickedImage.waitForExistence(timeout: elementWaitingTime))
  }
