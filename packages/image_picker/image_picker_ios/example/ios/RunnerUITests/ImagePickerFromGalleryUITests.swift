...
class ImagePickerFromGalleryUITests: XCTestCase {
  ...
  private func dismissKeyboardIfPresent() {
    if app.keyboards.element(boundBy: 0).exists {
      if UIDevice.current.userInterfaceIdiom == .pad {
        ...
        let predicate = NSPredicate(format: "label == 'Done' OR label == 'Hide keyboard'")
        let doneButton = app.buttons.matching(predicate).firstMatch
  ...
  func testCancel() {
    ...
    XCTAssertTrue(
      cancelButton.waitForExistence(timeout: elementWaitingTime), "Cancel button in gallery not found"
    )
  ...
  private func inputParameters(maxWidth: Int?, maxHeight: Int?, quality: Int?) {
  ...
  private func selectImageFromPicker() {
  ...
  func launchPickerAndPick(maxWidth: Int?, maxHeight: Int?, quality: Int?) {
    ...
    inputParameters(maxWidth: maxWidth, maxHeight: maxHeight, quality: quality)
