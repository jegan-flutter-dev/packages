...
class ImagePickerFromGalleryUITests: XCTestCase {
    ...
    if app.keyboards.element(boundBy: 0).exists {
      if UIDevice.current.userInterfaceIdiom == .pad {
        ...
        let doneButton = app.buttons.matching(
          NSPredicate(format: "label == 'Done' OR label == 'Hide keyboard'")).firstMatch
  ...
  func testCancel() {
    ...
    XCTAssertTrue(
      cancelButton.waitForExistence(timeout: elementWaitingTime),
      "Cancel button in gallery not found")
    ...
    XCTAssertTrue(
      unpickedText.waitForExistence(timeout: elementWaitingTime), "Expected 'not yet picked' text")
  }
  ...
  func launchPickerAndPick(maxWidth: Int?, maxHeight: Int?, quality: Int?) {
    ...
    tapPickButtonAndVerifyGallery()
    let unpickedPhoto = app.images.matching(
      NSPredicate(format: "label CONTAINS 'Photo' AND NOT (label CONTAINS 'picked')")).firstMatch
    if !unpickedPhoto.waitForExistence(timeout: 20) {
      handlePermissionInterruption()
    }
    XCTAssertTrue(
      unpickedPhoto.waitForExistence(timeout: elementWaitingTime), "No images found in picker")
    let cancelButton = app.buttons["Cancel"].firstMatch
    ...
    if cancelButton.exists && !cancelButton.waitForNonExistence(timeout: 10) {
      let dismissButton = app.buttons.matching(
        NSPredicate(format: "label CONTAINS 'Add' OR label CONTAINS 'Done'")).firstMatch
